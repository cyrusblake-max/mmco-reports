...
const TUNE_SCRIPT = `
  ...
  if (strategySection) {
    ...
    strategySection.style.backgroundColor = '#0A0A0A'
    const strategyInner = strategySection.querySelector('.max-w-6xl')
    if (strategyInner) strategyInner.style.backgroundColor = '#0A0A0A'
  }
  ...
  if (agentContactFlex && agentContactFlex.parentNode) {
    ...
    sigOuter.style.cssText = 'background-color:#0A0A0A;color:#FAFAF7;padding:0;break-inside:avoid;page-break-inside:avoid;'
